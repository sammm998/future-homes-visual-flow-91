-- Let's be more systematic. First, identify actual conflicts
WITH potential_conflicts AS (
  SELECT 
    tr_posts.id as tr_id,
    tr_posts.slug as tr_slug,
    REPLACE(tr_posts.slug, '-tr', '') as new_slug,
    base_posts.id as base_id
  FROM blog_posts tr_posts
  LEFT JOIN blog_posts base_posts ON base_posts.slug = REPLACE(tr_posts.slug, '-tr', '')
  WHERE tr_posts.slug LIKE '%-tr'
    AND base_posts.id IS NOT NULL
)
-- Delete the conflicting -tr posts to avoid duplicates
DELETE FROM blog_posts 
WHERE id IN (SELECT tr_id FROM potential_conflicts);

-- Now update remaining -tr posts
UPDATE blog_posts 
