CREATE OR REPLACE FUNCTION public.get_top_properties(days_back integer DEFAULT 30, top_n integer DEFAULT 20)
...
AS $function$
  WITH seg AS (
    SELECT unnest(ARRAY[
      'property','properties','fastighet','fastigheter','mulk','aqar','عقار','nedvizhimost',
      'eiendom','ejendom','melk','jaidad','propiedad','immobilie','propriete','propriete',
      'propriete','properti','emlak','gayrimenkul'
    ]) AS s
  ),
  parsed_page_views AS (
    SELECT
      ...
      count(*) AS c
    FROM (
      SELECT trim(both '/' from split_part(split_part(page, '?', 1), '#', 1)) AS clean_page
...
GRANT EXECUTE ON FUNCTION public.get_top_properties(integer, integer) TO anon, authenticated, service_role;