-- Handle conflicting slugs by adding a suffix for duplicates that would be created
-- First, check for posts without -tr that would conflict
UPDATE blog_posts 
SET slug = CASE 
  WHEN slug = 'turkish-culture-tr' THEN 'turkish-culture-guide'
  WHEN slug = 'healthcare-system-tr' AND 
       EXISTS (SELECT 1 FROM blog_posts b2 WHERE b2.slug = 'healthcare-system' AND b2.id != blog_posts.id) 
       THEN 'healthcare-system-guide'
  WHEN slug = 'property-purchase-expenses-tr' AND 
       EXISTS (SELECT 1 FROM blog_posts b2 WHERE b2.slug = 'property-purchase-expenses' AND b2.id != blog_posts.id) 
       THEN 'property-purchase-expenses-guide'
  ELSE REPLACE(slug, '-tr', '')
END
WHERE slug LIKE '%-tr';

-- Now we can safely add the purchase expenses article
