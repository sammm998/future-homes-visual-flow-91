-- Let's handle this more carefully by looking at each duplicate and deciding which to keep
-- First, handle the turkish-culture duplication by keeping only the latest one
WITH duplicate_posts AS (
  SELECT id, slug, ROW_NUMBER() OVER (PARTITION BY slug ORDER BY created_at DESC) as rn
  FROM blog_posts
  WHERE slug IN (
    SELECT slug 
    FROM blog_posts 
...
-- Now update slugs to remove -tr suffix
