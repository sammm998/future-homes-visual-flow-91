...
const CurrencySelector: React.FC = () => {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="font-medium text-xs">{String(selectedCurrency.code).toUpperCase().substring(0, 3)}</span>
          ...
          <div className="absolute top-full left-0 mt-1 w-20 bg-background border border-border rounded-lg shadow-lg z-50">
            <div className="p-1">
              {currencies.map((currency) => (
                ...
                >
                  <span className="font-medium text-xs">{String(currency.code).toUpperCase().substring(0, 3)}</span>
                </button>
              ))}
            </div>
          </div>
