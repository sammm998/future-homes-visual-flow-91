...
const CurrencySelector: React.FC = () => {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="font-medium text-xs" lang="en" translate="no">{selectedCurrency.code.slice(0, 3).toUpperCase()}</span>
          ...
          <div className="absolute top-full left-0 mt-1 w-20 bg-background border border-border rounded-lg shadow-lg z-50">
            <div className="p-1">
              {currencies.map((currency) => (
                ...
                >
                  <span className="font-medium text-xs" lang="en" translate="no">{currency.code.slice(0, 3).toUpperCase()}</span>
                </button>
              ))}
            </div>
          </div>
