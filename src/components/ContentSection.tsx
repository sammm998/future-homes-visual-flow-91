...
export const ContentSection: React.FC<ContentSectionProps> = ({ section, className = '' }) => {
  ...
  const renderContentSection = () => (
    <div className={`mb-16 ${className}`}>
      ...
      {section.description && (
        <p className="text-muted-foreground text-center max-w-4xl mx-auto mt-4">
          {section.description}
        </p>
      )}
    </div>
  ...
  const renderCompanyInfoSection = () => (
    <div className={`bg-primary/5 rounded-2xl p-8 mb-16 ${className}`}>
  ...
  const renderTeamHighlightSection = () => (
    <div className={`mb-16 ${className}`}>
  ...
  const renderFormSection = () => (
    <div className={`mb-16 ${className}`}>
      {section.content && (
        <p className="text-lg text-muted-foreground text-center">{section.content}</p>
      )}
    </div>
  );
  ...
  const renderPropertiesSection = () => (
    <div className={`mb-16 ${className}`}>
      <div className="text-center mb-12">
        {section.content && (
          <h2 className="text-3xl font-bold text-foreground mb-4">{section.content}</h2>
        )}
        {section.description && (
          <p className="text-lg text-muted-foreground">{section.description}</p>
        )}
    ...
    return (
      <div className={`mb-16 ${className}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {section.offices.map((office: any, index: number) => (
