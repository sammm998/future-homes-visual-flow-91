...
const FrancePropertySearch = () => {
  ...
  useEffect(() => {
    const urlFilters: PropertyFilters = {
      ...
      location: searchParams.get('location') || 'France',
    ...
    const hasFilters = Object.entries(urlFilters).some(([key, value]) => {
      return value && value !== '' && value !== 'ref' && value !== 'France';
    });
    
    setShowFiltered(hasFilters);
  }, [searchParams, location.state]);
  
  // Filter properties to only show France properties
  const franceProperties = useMemo(() => {
    return allProperties.filter(property => 
      property.location?.toLowerCase().includes('france') || 
      property.location?.toLowerCase().includes('strasbourg')
    ).map(property => {
      ...
      return {
        ...
        coordinates: [48.5833, 7.7500] as [number, number] // Default Strasbourg coordinates
      };
    });
  }, [allProperties]);
  ...
  const filteredProperties = useMemo(() => {
    if (showFiltered) {
      return filterProperties(franceProperties, filters);
    }
    return franceProperties;
  }, [franceProperties, filters, showFiltered]);
  ...
  const handleFilterChange = (newFilters: PropertyFilters) => {
    ...
    const hasFilters = Object.entries(newFilters).some(([key, value]) => {
      ...
      return value && value !== '' && value !== 'France';
    });
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEOHead 
        title="Properties For Sale in France - Future Homes"
        description="Discover luxury apartments and properties for sale in France, Strasbourg. Modern amenities in the heart of Europe."
        keywords="France properties, Strasbourg real estate, France apartments, properties for sale France"
      />
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-xl text-muted-foreground">
              {franceProperties.length} properties found
            </p>
          </div>
          ...
          {showFiltered && (
            <div className="mb-4 text-center">
              <p className="text-sm text-muted-foreground">
                Showing {filteredProperties.length} of {franceProperties.length} properties
              </p>
              <Button 
                variant="outline" 
                size="sm" 
                ...
                content: (
                  <div>
                    <p className="text-foreground text-xs md:text-sm font-normal mb-4">
                      Premium property with modern amenities and excellent location in France.
                    </p>
                    <div className="mb-6">
